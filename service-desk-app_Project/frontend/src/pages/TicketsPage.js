...
const TicketsPage = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleteLoading(ticketToDelete._id);

    try {
      await axios.delete(
        `${process.env.REACT_APP_API_BASE}/api/tickets/${ticketToDelete._id}`
      );
  ...
  const applyFilters = () => {
    ...
    if (filters.categoryFilter !== "all") {
      filtered = filtered.filter(
        (ticket) => ticket.category === filters.categoryFilter
      );
    }

    if (filters.priorityFilter !== "all") {
      filtered = filtered.filter(
        (ticket) => ticket.priority === filters.priorityFilter
      );
    }
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-end space-x-3">
                    <button
                      ...
                      className="px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-all duration-200"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={handleDeleteConfirm}
                      disabled={deleteLoading === ticketToDelete?._id}
                      className="px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                    >
                      {deleteLoading === ticketToDelete?._id ? (
                        <>
                          <svg
                            className="animate-spin -ml-1 mr-2 h-4 w-4 text-white"
                            fill="none"
            ...
            <div className="p-6">
              <div className="mb-4">
                <div className="relative">
                  <svg
                    className="bg-transparent absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {filters.priorityFilter !== "all" && (
                  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                    ...
                    <button
                      onClick={() =>
                        handleFilterChange("priorityFilter", "all")
                      }
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          {(role === "admin" ||
                            ...
                            <button
                              ...
                              className="px-3 py-1 border border-red-500 text-red-600 rounded-lg hover:bg-red-100 transition-colors text-sm font-medium"
                            >
                              Delete
                            </button>
                          )}
