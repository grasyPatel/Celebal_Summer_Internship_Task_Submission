...
const AdminTicketsPage = () => {
  ...
  // Only send email if we have the necessary ticket information
  ...
  // Use the imported emailjs directly (no need to check window.emailjs)
  const emailParams = {
  ...
  // Use the imported emailjs directly
  emailjs.send(
    EMAILJS_SERVICE_ID,
    EMAILJS_TEMPLATE_ID,
    emailParams,
  ...
  .catch((error) => {
    ...
    // Don't throw error to prevent breaking the status update functionality
  });
};
  ...
  const handleStatusChange = async (ticketId, newStatus) => {
    ...
    try {
      ...
      // Find the ticket to get its details for email
